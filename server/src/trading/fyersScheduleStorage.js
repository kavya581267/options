...
function schedulePath() {
  return path.join(config.dataDir, 'fyers-schedule.json');
}
...
export async function loadSchedule() {
  try {
    ...
    if (err.code !== 'ENOENT') {
      console.warn('[fyers-schedule] load failed:', err.message);
    }
    schedule = { ...DEFAULT_SCHEDULE, symbol: config.fyersTrading.symbol };
  }
  return getSchedule();
}
